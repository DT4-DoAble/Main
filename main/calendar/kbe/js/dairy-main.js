document.addEventListener("DOMContentLoaded", function () {
  ...
  const calendarHTML = function (
    ...
    let html = `<ul class="dairy-item-list">`;
    let calendarPos = 0;
    let calendarDay = 0;
    for (let day = 0; day < 42; day++) {
      html += `<li id="day-sell-${calendarDay + 1}" class="day-sell">`;
      html += `<h3 class="dairy-item-title">`;
      if (monthStartDay <= calendarPos && calendarDay < monthLastDate) {
        calendarDay++;
        html += `<span class="day-number text-xl-b ${
          showToday &&
      ...
      html += `<img class="emoji-img" src="./img/emotion-emoji-s-bad.svg" alt="emotion emoji">`;
      html += "</h3>";
      html += `<span class="tag tag-s-s tag-c-r text-b-m">따분한</span>`;
      html += "</li>";
      calendarPos++;
    }
    html += "</ul>";
    ...
    try {
      ...
      if (calendars.length > 0) {
        calendars.forEach((item) => {
          ...
          memoTag.setAttribute(
            "class",
            `memoList contents text-xs-r`
          );
          document.querySelector(`#day-sell-${calendarDay}`).append(memoTag);
        });
      }
    } catch (e) {
